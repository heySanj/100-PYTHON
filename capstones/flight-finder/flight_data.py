class FlightData:
    ...
    def __init__(self, data):
        ...
        print(data['route'][1]['local_arrival'])
        self.dep_date = (data['route'][0]['local_departure']).split("T")[0]
        self.ret_date = (data['route'][1]['local_arrival']).split("T")[0]
        
    def get_details(self):
        message = f"""
        Low price alert! Only ${self.price} to fly from {self.cityFrom}-{self.flyFrom}
        to {self.cityTo}-{self.flyTo}, from {self.dep_date} to {self.ret_date}.
        """
        return message