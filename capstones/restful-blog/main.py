...
def new_post():
    ...
    # Save the new post to database on submit
    if new_post_form.validate_on_submit():
        time_now = dt.now()
        blog_post = BlogPost(
            title = new_post_form.title.data,
            subtitle = new_post_form.subtitle.data,
            date = f"{time_now.strftime('%B')} {time_now.strftime('%d')}, {time_now.strftime('%Y')}",
...
def edit_post(post_id):    
    # Get the post details and fill out the edit form
    ...
    # When the updates are submitted -> update the database
    ...
    return render_template("make-post.html", form=edit_post_form, post_id=post_id)
